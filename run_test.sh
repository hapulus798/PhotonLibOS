...
for iodepth in "${iodepth_values[@]}"; do
    for bs in "${bs_values[@]}"; do
        ...
        { sudo ./build/output/test-spdknvme --gtest_filter="SPDKNVMeTest.performance2" --gtest_color=no --bs=$bs --iodepth=$iodepth --iswrite=true 2>&1 1>&3 | tee ./perftest2_write2/${iodepth}_${bs}.txt | head -n 40 > /dev/null; } 3>&1
    done
done